...
function normalize (item) {
  return { title: text(item.content), url: item.url || null, children: (item.items || []).map(normalize) }
}
...
function collectItems (contentCatalog, navigationCatalog, inc, focus, report) {
  ...
  resolveComponents(contentCatalog, navigationCatalog, inc, focus).forEach(({ name, version, title }) => {
    var perComponent = topLevelItems(navigationCatalog, name, version).filter((n) => matchesUrl(n, inc.urlContains))
